...
If you are working with a team, your whole team should discuss how you want to stay consistent, particularly with respect to how you want to translate important words, phrases, and idioms from the source text. You may want to discuss how you can use punctuation consistently across your team, and that in lines of text you identify spaces that should not allow the line to break, such as spaces that surround puntuation that should not start a line. Dashes are a good example: if you are writing something like "phrase⍽—␣second phrase", you don't want the dash to start the line, so the first space should be a non-breaking space, while the second is not.
