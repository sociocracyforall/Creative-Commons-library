...
<xsl:stylesheet
  ...
  <!-- Override the built-in template to add a short table of contents ... or maybe not? -->
  <xsl:template match="book|article" mode="toc_lots_disable">
    <xsl:if test="$doc.toc.show != '0'">
      <xsl:text>\shorttoc{Matières}{1}&#10;</xsl:text>
